...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		...
		<script type="text/javascript">
			...
			function batchDelete(pageNo,name,status) {
				...
				if(size>0) {
					if(confirm("确定要删除吗?")) {
						$("#delForm").attr("action","/batchDelete.do?name=" + name + "&status=" + status + "&pageNo=" + pageNo).submit();
					}
				}else {
					return;
				}
	...
	<body>
		...
		<div class="body-box">
			<form action="brand.do" method="post" style="padding-top:5px;">
				...
				<tbody class="pn-ltbody">
					<c:forEach items="${pageInfo.list}" var="brand">
						<tr bgcolor="#ffffff" onmouseout="this.bgColor='#ffffff'" onmouseover="this.bgColor='#eeeeee'">
							...
							<td align="center"><img width="40" height="40" src="${brand.realUrl}" /></td>
							...
							<td align="center">
								...
								<a class="pn-opt" onclick="if(!confirm('您确定删除吗？')) {return false;}" href="/batchDelete.do?id=${brand.id}&name=${name}&pageNo=${pageNo}&status=${status}" >删除</a>
							</td>
						</tr>
					</c:forEach>
				</tbody>
